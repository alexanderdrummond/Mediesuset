...
const CampModal = ({ camp, onClose }) => {
  ...
  const partoutTickets = camp.tickets.filter(ticket => ticket.type === 'partout');
  const singleTickets = camp.tickets.filter(ticket => ticket.type === 'single');


  const renderTicketCards = (tickets) => tickets.map(ticket => (
    <div key={ticket.id} className={style.ticketCard}>
      <h3>{ticket.name}</h3>
      <p>{ticket.description}</p>
      <p>Price: {ticket.price} DKK</p>
    </div>
  ));
